...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    case 'saml':
      ...
      break;
    case 'jwt':
      strategy = new AuthStrategy(_.defaults({}, options),
        function (req, jwtPayload, done) {
          jwtPayload.id = jwtPayload.sub; //sub attribute is primary id
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(req.user.id, provider, authScheme, jwtPayload, {}, options, done);
            } else {
              done(g.f('No user is logged in'));
            }
          } else {
            self.userIdentityModel.login(provider, authScheme, jwtPayload, {}, options, loginCallback(req, done));
          }
        }
      );
      break;
  ...
  if (authType === 'local') {
    ...
    self.app.post(authPath, passport.authenticate(
      name, options.fn || _.defaults({
        ...
        failureFlash: options.failureFlash,
        scope: scope, session: session,
      }, options.authOptions)), options.customCallback || defaultCallback);
